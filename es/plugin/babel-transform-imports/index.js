...
function barf(msg) {
    throw new Error('babel-transform-imports: ' + msg);
}
        ...
        visitor: {
            ImportDeclaration: function ImportDeclaration(path, state) {
                ...
                if (!(source in state.opts) && source.match(/^\.{0,2}\//)) {
                    source = pathLib.resolve(pathLib.join(source[0] === '/' ? '' : pathLib.dirname(state.file.opts.filename), source));
                }
                ...
                if (source in state.opts) {
                    ...
                    memberImports.forEach(function (memberImport) {
                        ...
                        var newImportSpecifier = opts.skipDefaultConversion ? memberImport : types.importDefaultSpecifier(types.identifier(memberImport.local.name));

                        transforms.push(types.importDeclaration([newImportSpecifier], types.stringLiteral(replace)));
