...
URL.para2Obj = function (urlParam) {
	var data = {};
	var urlParamArr = (urlParam + '').match(/([^=&#\?]+)=[^&#]+/g) || [];
	//必须是合法字符串
	if (urlParamArr.length) {
		urlParamArr.forEach(function (para, i) {
			var d = para.split("="),
...
URL.getPara = function (url, paraName, isHash) {
	...
	var paraObj = URL.para2Obj((url + "").split("#")[isHash ? 1 : 0]);
...
URL.removePara = function (url, paraName) {
	...
	paraReg = paraReg.map(function (p) {
...
URL.addPara = function (href, para, removeSamePara) {
	...
	//先移除同名参数
	if (removeSamePara) {
		url[0] = URL.removePara(url[0], (para.match(/([^=&#\?]+)=[^&#]+/g) || []).map(function (str) {
			return str.replace(/=.+$/, "");
		}));
	}
...
URL.getInfo = function (url) {
	var fixUrl = URL.getFullPath(url || location.href),
	    info = /^((\w+:)\/\/)?(?:(\w+):?(\w+)?@)?([^\/\?:]+):?(\d+)?(\/?[^\?#]+)?(\??[^#]+)?(#?.*$)/.exec(fixUrl) || [];
	info = info.map(function (v) {
		return v || '';
	});
	var portStr = info[6] ? ':' + info[6] : '';
	return {
		href: info[0],
...
URL.getFullPath = function (url, basePath) {
	...
	//开始处理和转化
	var locationInfo = URL.getInfo(basePath);
	var port = locationInfo.port || "80",
	    fromRoot = /^\//.test(url);
	if (!fromRoot) {
