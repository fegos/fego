...
export default class ListItem extends Component {
	...
	onTouchMove = e => {
		...
		let touchEvent = e.nativeEvent;
		let touch = touchEvent.changedTouches[0];
		let diffScreenY = touch.screenY - this.touchEvent.screenY;
		if (Math.abs(diffScreenY) > touchEvent.target.offsetHeight) return;
		
		let diffScreenX = touch.screenX - this.touchEvent.screenX;
		let left = diffScreenX;

		if (diffScreenX > 0) { // ➡️右滑，直接设为 0
			left = 0;
		} else {
			if (diffScreenX < 0 - this.width || this.state.expand) { // ⬅️左滑，但超过了宽度，或是已展开的情况下左滑，都维持最大宽度
				left = 0 - this.width;
			}
		}
	...
	onTouchEnd = e => {
		...
		let { moving, expand } = this.state;
		// 滑动中->滑动结束
		if (moving) {
			let diffScreenX = touch.screenX - this.touchEvent.screenX;
			let diffScreenY = touch.screenY - this.touchEvent.screenY;
			let left = diffScreenX, expand;
			...
			let diffPageX = touch.pageX - this.touchEvent.pageX;
			let diffPageY = touch.pageY - this.touchEvent.pageY;
	...
	handleSlipTouchEnd = e => {
		let { onSlipItemClick } = this.props;
	...
	render() {
		let { prefixCls, title, subTitle, wrap, hasRightArrow, extra, verticalAlign, iconName, slip, children, className, highlight, link, style } = this.props;
		// 处理有左滑时候的样式
		let finalStyle = {};
		if (slip) {
			finalStyle.left = this.state.left + 'px';
		}
		let TagName = link ? Link : 'div';
		let extraProps = link ? {to: link} : {};
		return (
			<TagName className={classNames(`${prefixCls}-item`, className, {
