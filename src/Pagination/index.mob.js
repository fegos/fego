...
export default class Pagination extends Component {
	...
	constructor(props) {
		...
		let { page, defaultPage, defaultPageSize, total } = props,
	...
	componentWillReceiveProps(nextProps) {
		let { page, total } = nextProps, _state = {};
	...
	render() {
		let { prefixCls, className, style, size, total, prevText, nextText } = this.props,
			{ curPage, totalPage } = this.state,
			prevBtnDisabled = totalPage === 0 || curPage === 1,
			nextBtnDisabled = totalPage === 0 || curPage === totalPage;

		if (total < 0) return null;

		return(
			<div unselectable="unselectable" className={classNames(prefixCls, className, {
